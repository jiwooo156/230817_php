...
class BoardController extends Controller
{
    public function index() {
        ...
        // 카테고리가 4, 7, 8인 게시글의 수를 출력하시오
        $result = DB::select('select count(id) from boards where categories_no = ? or categories_no = ? or categories_no = ?', [4, 7, 8]);

        // 카테고리별 게시글 갯수를 출력하시오
        $result = DB::select('select categories_no, count(categories_no) from boards group by categories_no');
        $result = DB::select('SELECT c.no, c.name, count(c.no)cnt FROM boards b JOIN categories c ON b.categories_no = c.no GROUP BY c.no, c.name');
        ...
        $result = DB::table('boards')->select('boards.title','boards.content','categories.name')->join('categories','categories.no','boards.categories_no')->limit(100)->get();

        // 카테고리별 게시글 갯수와 카테고리명을 출력해 주세요
        $result = DB::table('boards')->select('categories.no','categories.name', DB::raw('count(categories.no)'))->join('categories','categories.no','boards.categories_no')->groupBy('categories.no', 'categories.name')->get();
