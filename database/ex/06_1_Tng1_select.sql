...
SELECT emp.emp_no, emp.first_name, emp.last_name, emp.gender FROM employees AS emp where emp_no IN (SELECT emp_no FROM salaries AS sal GROUP BY sal.emp_no HAVING AVG (sal.salary) >= 70000);
-- 10. 현재 직책이 "Senior Engineer"인, 사원의 사원번호와 성을 조회해 주세요.
SELECT emp.emp_no, emp.last_name FROM employees AS emp WHERE emp.emp_no IN (SELECT tit.emp_no FROM titles AS tit WHERE tit.title = 'senior engineer' AND to_date >= 20230904) ;