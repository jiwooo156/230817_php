...
-- 	- 검색하고자 하는 데이ㅐ터가 테이블의 10~15% 이하 일 경우 가장 효율적
-- 	- 속도 향상을 위해서는 우선 쿼리를 좀 더 효율적으로 짜는 방향을 고려(인덱스는 최후의 수단)
-- 	- 인덱스를 추가 했다면 대량의 데이터로 해당 테이블의 CRUD를 테스트
-- 	- 사용하지 않는 인덱스는 제거
-- 	- FK를 지정한 열을 자동으로 FK 인덱스가 생성(*DBMS에 따라 상이하므로 주의)

-- 5. 인덱스를 사용하기 좋은 상황
-- 	- 규모가 작지 않은 테이블 (100만 넘어가면 규모가 큰거임.)
-- 	- INSERT, UPDATE, DELETE가 자주 발생하지 않는 칼럼
--		- JOIN이나 WHERE 또는 ORDER BY 에 자주 사용되는 칼럼 
-- 	- 데이터의 중복도가 낮은 칼럼
