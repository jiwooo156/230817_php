...
-- 		(테이블을 식별하기 위함)(insert,update,delete를 할 때 데이터의 기준점이 됨)

-- 	- Foreign Key(FK)
-- 		(join 같은 참조관계에 주는 제약조건. 함부로 삭제 못 하게함)
...
-- 		> 고유 인데스를 자동 생성합니다.
-- 		(pk는 아니지만(pk는 id) 회원아이디도 중복되면 안되기때문에 unique로 설정해줌)
	
-- 	- NULL / NOT NULL
-- 		> 해당 컬럼의 NULL 값 허용 여부를 설정합니다.

-- 	- DEFAULT
-- 		(insert할 때만 영향감. 값을 안넣어도 자동으로 기본값 들어감.)
-- 		> 해당 컬럼에 기본값을 설정합니다.

-- 	- CHECK
-- 		(insert, update에 영향있음. 근데 보통 php에서 처리함.)
